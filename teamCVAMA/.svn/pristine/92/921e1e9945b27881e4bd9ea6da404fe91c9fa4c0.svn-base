...
var replyService = (function(){
	...
	//댓글삭제
	function rdelete(reply, callback, error){
// 		alert("댓글 삭제:reply:"+JSON.stringify(reply));
		$.ajax({
			type : "post",
			url : '/reply/delete',
			data : JSON.stringify(reply),
...
$(function(){
	...
	// 댓글 달기 취소 버튼 처리 -> 모달 창을 보이게 한다.
	$("#updateModal_cancelBtn").click(function(){
		// 댓글 등록으로 제목과 버튼이름을 바꿔야 한다.
		$("#updateModalTitle").text("댓글 등록");
		$("#updateModal_updateBtn").text("등록");
		$("#modal_writer").attr("readonly", false);		
...
//	삭제 처리
	$("#deletemodal_form").submit(function(){
		var reply ={
				rno : $("#deletemodal_rno").val(),
				pw : $("#deletemodal_pw").val()
			}
//  			alert(JSON.stringify(reply));
			replyService.rdelete(
					reply,
					function(result){
						//결과를 경고창으로 보여준다
// 						alert("RESULT:"+result);
            ...
            <div class="btn-group right">
             <button type="submit" class="btn btn-primary"
              id="updateModal_updateBtn">등록</button>
          	 <button type="button" class="btn btn-warning" data-dismiss="modal" id="updateModal_cancelBtn"><span class="glyphicon glyphicon-remove"></span> 취소</button>
			</div> 
          </form>
        </div>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header" style="padding:35px 50px;">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4><span class="glyphicon glyphicon-pencil"></span> 
          	<span id="deleteModalTitle">댓글 삭제</span>
          </h4>
        </div>
        <div class="modal-body" style="padding:40px 50px;">
          <form role="form" method="post" id="deletemodal_form">
          	<input type="hidden" name="rno" id="deletemodal_rno">
            <input name="no" type="hidden" value="${viewDTO.no }" id="deletemodal_no"/>
			<input name="page" type="hidden" value="${param.page }" />
			<input name="perPageNum" type="hidden" value="${param.perPageNum }" />
            <div class="form-group">
              <label for="modal_content">
              ...
              <input class="form-control" id="deletemodal_pw" name="pw">
              <div>본인 댓글 확인 비밀번호입니다.</div>
            </div>
            <div class="btn-group right">
             <button type="submit" class="btn btn-primary"
              id="deleteModal_deleteBtn">삭제</button>
          	 <button type="button" class="btn btn-warning" data-dismiss="modal" id="deleteModal_cancelBtn"><span class="glyphicon glyphicon-remove"></span> 취소</button>
			</div> 
          </form>
        </div>
